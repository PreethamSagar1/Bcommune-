...
async def reason_over_resumes(query_text: str, resumes: List[dict]) -> str:
    """Use GPT-4 to reason over retrieved resumes and generate insights."""
    system_prompt = """
...
async def plan_next_steps(query_text: str, resumes: List[dict]) -> str:
    """Use GPT-4 to decide the next steps based on the retrieved resumes."""
    system_prompt = """
    You are an AI assistant that helps with resume searches please do not make any assumptions only state clear facts.
...
async def store_interaction(query_text: str, resumes: List[dict]):
    """Store the search query and retrieved resumes in memory."""
    memory.append({
        "query": query_text,
        "resumes": resumes,
...
def streamlit_ui():
    st.title("AI-Powered Job Recommendation System with Agentic RAG")
    ...
    if page == "Upload Resume":
        ...
        st.header("Search Resumes with Agentic RAG")
        ...
        if st.button("Search Resumes"):
            if search_query:
                ...
                if resumes:
                    ...
                    asyncio.run(store_interaction(search_query, resumes))
                else:
